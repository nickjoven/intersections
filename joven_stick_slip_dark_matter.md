...
The conditional is important and we state it explicitly: *if* the gravitational field at each radius can be described as solving a locally-defined constrained optimization — with the dual variable at each point set by the local constraint violation — *then* the following derivation is exact. This conditional is now discharged: the companion paper ([Joven, 2026, "Gravity Is Synchronization Through a Frictional Medium"](https://github.com/nickjoven/201)) identifies the optimization structure as the Einstein-Hilbert action itself in ADM form, with the Hamiltonian constraint at each radius providing exactly the locally-defined constrained optimization required. The Renzo's Rule derivation therein establishes both forward and inverse directions from this variational foundation.
...
The companion notebook demonstrates the mechanism schematically — threshold produces subharmonic, a dual-variable iteration converges or diverges — but on a simplified model where the primal variables are fixed and each radius is independent. This is not Lagrangian relaxation in the full sense: there is no inner optimization, no coupling between radial shells, and the dual variable converges to the known deficit by construction. The gap between this schematic and a theory was originally threefold: (1) choosing a specific geometric constraint (what is the gravitational field optimizing against?); (2) formulating the genuine dual problem with coupled primal-dual dynamics, verifying that strong duality holds (Ekeland & Temam, 1976) and that the shadow price interpretation is rigorous; and (3) solving the coupled problem on observed baryonic mass profiles to produce predicted rotation curves without free parameters beyond the constraint specification.

Items (1) and (2) are now discharged. The companion paper ([Joven, 2026, "Gravity Is Synchronization Through a Frictional Medium"](https://github.com/nickjoven/201)) identifies the geometric constraint as the Einstein-Hilbert action in ADM form, with the Hamiltonian constraint emerging as a KKT stationarity condition rather than being imposed artificially. The Kuramoto self-consistency equation provides the missing fixed-point structure linking metric evolution to dark matter density, establishing the coupled primal-dual dynamics. Complementary slackness emerges from threshold behavior in Kuramoto synchronization rather than appearing as an external assumption. Only item (3) — solving the coupled problem on observed baryonic mass profiles to produce predicted rotation curves without free parameters — remains open.
...
### 10.7 Cylinder Stick-Slip: A Tabletop QPO Analogue

The paper's mechanism is built on a bowed string — a one-dimensional medium with fixed endpoints. An accretion disk is a two-dimensional medium with periodic boundary conditions. The cylinder is the geometry that bridges them. The companion notebook (`09_cylinder_stick_slip.ipynb`) develops this in detail; the key results are summarized here.

A thin-walled cylindrical shell supports circumferential modes indexed by azimuthal order $m$. The $m$-th mode has $2m$ nodal meridians equally spaced around the circumference. The $m = 2$ mode has nodal meridians every 90°; the $m = 3$ mode every 60°. The node-antinode collision described in §5.6 for the string transfers directly: between any two adjacent $m = 2$ nodes (separated by 90°), the $m = 3$ mode places nodes at 1/3 and 2/3 of the interval — exactly the positions that are $m = 2$ antinodes. The 3:2 frequency ratio survives the promotion from one dimension to two, from Dirichlet to periodic boundary conditions.

The cylinder adds something the string cannot provide: **traveling waves**. A standing wave $\cos(m\theta)$ decomposes into counter-rotating traveling waves $e^{\pm im\theta}$. On a string, the stick-slip subharmonic is a standing wave — a tone. On a cylinder, it can propagate as a rotating pattern. A quasi-periodic oscillation in an accretion disk IS a traveling density wave rotating at a frequency different from the material's orbital frequency. If a cylinder produces a traveling stick-slip subharmonic, it is, structurally, a QPO.

Circumferential stick-slip already exists in familiar systems. A wine glass excited by a rubbed rim is a bowed cylinder: the finger alternates between stick and slip tangentially, exciting the $m = 2$ circumferential mode. Singing bowls, brake disks, and railway wheels all exhibit the same mechanism at various mode orders. The question is not whether a cylinder can stick-slip — it can and does — but whether the subharmonic regime is accessible: whether slowing the tangential drive (or increasing the contact pressure) causes the $m = 3$ mode to activate alongside $m = 2$, producing a 3:2 frequency ratio.

**The experimental proposal.** Mount a thin-walled brass or aluminum tube on bearings. Press a rosined pad tangentially against the outer surface. Rotate the tube at variable speed. The control surface maps from §1.2: rotation speed is the drive parameter, contact pressure is the coupling strength. Measure the acoustic output with a microphone and FFT. At normal rotation speeds, the tube should sing in the $m = 2$ mode (wine-glass regime). Below a critical speed (or above a critical pressure), the prediction is that the $m = 3$ mode activates, the frequency spectrum shows two peaks at a 3:2 ratio, and — critically — the mode pattern should be a traveling wave rotating around the tube, not a standing wave pinned to the contact point. If this is observed, it constitutes a tabletop QPO generator: the same mechanism (circumferential stick-slip), the same ratio (3:2 from nodal geometry), the same wave type (traveling), at a scale accessible to any acoustics laboratory.

The accretion disk introduces one complication the cylinder lacks: differential rotation. On a rigid cylinder, all circumferential points share the same angular velocity. In a Keplerian disk, the angular velocity falls as $r^{-3/2}$. This means the shear rate (the drive) varies with radius, and the stick-slip threshold is crossed at a specific radius where the shear matches the medium's critical value. The azimuthal mode structure at that radius determines the QPO frequencies. But the mode numbers are integers — $m = 2$ and $m = 3$ — and the ratio of integers does not depend on the mass of the black hole. This is why the 3:2 ratio is mass-independent: the physical frequencies scale with mass, but the ratio is set by the geometry.
